...
>(
  ...
  if (user) {
    if (
      !user.metadata ||
      user.metadata[AUTH_PROVIDER_KEY] !==
        strategyNames[strategyErrorIdentifier].admin
    ) {
      throw new MedusaError(
  ...
  return await manager.transaction(async (transactionManager) => {
    ...
    if (customer) {
      ...
      if (
        ...
        customer.metadata[AUTH_PROVIDER_KEY] =
          strategyNames[strategyErrorIdentifier].store;
    ...
    type KobilCreateCustomerInput = CreateCustomerInput & { id: string };
